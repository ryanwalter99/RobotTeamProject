...
def main():
    ...
    root.bind('<w>', lambda event: forward(mqtt_client, left_speed_entry, right_speed_entry))

    look_button = ttk.Button(main_frame, text='look')
    look_button.grid(row=6, column=0)
    look_button['command'] = lambda: look(mqtt_client)
    root.bind('<space>', lambda event: look(mqtt_client))
    ...
    up_button = ttk.Button(main_frame, text='Arm Up')
    up_button.grid(row=5, column=0)
    up_button['command'] = lambda: arm_up(mqtt_client)
    root.bind('<q>', lambda event: arm_up(mqtt_client))

    down_button = ttk.Button(main_frame, text='Arm down')
    down_button.grid(row=5, column=0)
    down_button['command'] = lambda: arm_down(mqtt_client)
    root.bind('<q>', lambda event: arm_down(mqtt_client))

    exit_button = ttk.Button(main_frame, text='Robot Out!')
    exit_button.grid(row=6, column= 2)
    exit_button['command'] = lambda: exit(mqtt_client)
    root.bind('<p>', lambda event: exit(mqtt_client))

    speak_button = ttk.Button(main_frame, text='Speak')
    speak_button.grid(row=4, column=0)
    speak_button['command'] = lambda: speak(mqtt_client)
    root.bind('<e>', lambda event: speak(mqtt_client))
...
def speak(mqtt_client):
    print('Listen to me!')
    mqtt_client.send_message('speak')


def exit(mqtt_client):
    print('exit')
    mqtt_client.send_message('exit')
    mqtt_client.close()

def look(mqtt_client):
    print('looking for darkness')
    mqtt_client.send_message('look')
